...
const TEST_FILE = path.resolve(TEST_DIR, "products.json");
...
const tests = {
  ...
  changeAttributeTests: () => {
    ...
    // sellerId should not be updatable, categoryId should be updatable
    const updatedSeller = service.updateAttribute(1, "sellerId", 99);
    assert.strictEqual(updatedSeller, null);

    const updatedCategory = service.updateAttribute(1, "categoryId", 5);
  ...
  findByAttributeTests: () => {
    ...
    assert.strictEqual(found[0].getAttribute("name"), "Mouse");
  ...
  idValidationTests: async () => {
    ...
    assert.notStrictEqual(
      next,
      2,
      "JSON auto-increment must not reuse old IDs"
    );
...
const runAllTests = async () => {
  ...
  console.log("\nAll JSON ProductService tests passed successfully!");
};
