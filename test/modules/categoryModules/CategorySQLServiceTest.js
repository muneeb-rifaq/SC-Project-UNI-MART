...
const tests = {
  ...
  idValidationTests: async () => {
    console.log("\n=== ID VALIDATION TESTS ===");
    let prev = service.getNextAvailableID();
    service.eraseAll();
    ...
    assert.notStrictEqual(next, 2, "SQL auto-increment must not reuse old IDs");
...
const runAllTests = async () => {
  ...
  console.log("\nAll SQL CategoryService tests passed successfully!");
};
